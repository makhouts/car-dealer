...
import { Inbox, CheckCircle2, Mail, Phone, Car, Clock, Wrench } from 'lucide-react'
...
export default function AdminCarrosserieLeadsPage() {
    ...
    const fetchCounts = async () => {
        try {
            const [allRes, newRes, handledRes] = await Promise.all([
                fetch('/api/carrosserie-leads'),
                fetch('/api/carrosserie-leads?filter=new'),
                fetch('/api/carrosserie-leads?filter=handled'),
            ])
    ...
    const fetchLeads = async () => {
        ...
        try {
            const url =
                filter === 'all'
                    ? '/api/carrosserie-leads'
                    : `/api/carrosserie-leads?filter=${filter}`
                ...
                <div className="max-w-6xl mx-auto">
                    <div className="mb-8">
                        <div className="flex items-center gap-3 mb-1">
                            <Wrench className="w-7 h-7 text-neutral-700" />
                            <h1 className="text-3xl font-bold text-neutral-900">Carrosserie Leads</h1>
                        </div>
                        <p className="text-neutral-600 ml-10">Beheer carrosserie &amp; herstel aanvragen</p>
                    </div>
                        ...
                        {loading ? (
                            ...
                            <div className="p-12 text-center">
                                ...
                                <p className="text-neutral-500">
                                    {filter === 'new' && 'Geen nieuwe carrosserie leads'}
                                    {filter === 'handled' && 'Geen afgehandelde carrosserie leads'}
                                    {filter === 'all' && 'Nog geen carrosserie leads'}
                                </p>
                            </div>
                        ) : (
                            <div className="divide-y divide-neutral-100">
                                {leads.map((lead) => (
                                    <div
                                        key={lead.id}
                                        className={`p-6 hover:bg-neutral-50 transition-colors ${!lead.handled ? 'bg-red-50/30' : ''}`}
                                    >
                                        <div className="flex items-start justify-between mb-3">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 text-sm text-neutral-500">
                                                    ...
                                                    <span className={!lead.handled ? 'text-red-600 font-medium' : ''}>
